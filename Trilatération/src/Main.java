...
public class Main {
	...
	public static void main(String[] args) {
		Point A = new Point(0, 0, 2.5);
		Point B = new Point(0, 4, 2.5);
		Point C = new Point(3, 0, 2.5);
		
		Point M = M(A, B, C);
		System.out.println("La personne se trouve en X = " + M.getX() + " et Y = " + M.getY());
	}
    ...
    public static Point I1(Point A, Point B)
    {
		...
		if(Ya == Yb)
		{
			x = (Xa * Xa - Xb * Xb + Ya * Ya - Yb * Yb - da * da + db *db) / (2 * Xa - 2 * Xb);
			y = (2 * Ya - Math.sqrt(4 * (da * da - (x - Xa) * (x - Xa)))) / 2;
		}
		else
		{
			x = (-b(A, B) - Math.sqrt(D(A, B))) / (2 * a(A, B));
	        y = e(A, B) - d(A, B) * x;			
		}
